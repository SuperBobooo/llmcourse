...
def append_experiment_result(
    ...
    config.results_table_path.parent.mkdir(parents=True, exist_ok=True)
    file_exists = config.results_table_path.exists()
    ...
    row = {
        ...
        "top_k_experts": config.top_k_experts if config.use_moe else 0,
    ...
    with config.results_table_path.open("a", encoding="utf-8", newline="") as file:
...
def print_dataset_status(config: Config, metadata: dict | None) -> None:
    if metadata is None:
        print(
            "Dataset metadata not found. Training will use the TSV files already under "
...
def run_epoch(
    ...
    for batch in data_loader:
        batch = move_batch_to_device(batch, device=config.device)
        ...
        if config.use_moe and config.use_moe_aux_loss:
            total_batch_loss = total_batch_loss + config.moe_aux_loss_coef * aux_loss
...
def train_model(config: Config | None = None) -> dict:
    ...
    print(f"Experiment: {config.experiment_name} | Preset: {config.preset_name}")
    ...
    print(
        ...
        f"num_heads={config.num_heads} | num_kv_heads={config.num_kv_heads}"
    )
    print(
        f"FFN type: {config.ffn_type} | "
        f"use_moe={config.use_moe} | num_experts={config.num_experts if config.use_moe else 0} | "
        f"top_k_experts={config.top_k_experts if config.use_moe else 0}"
    )
    print(
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        train_loss, train_ce_loss, train_aux_loss, train_acc = run_epoch(
            model=model,
        ...
        with torch.no_grad():
            val_loss, val_ce_loss, val_aux_loss, val_acc = run_epoch(
                model=model,
        ...
        print(
            ...
            f"train_ce={train_ce_loss:.4f} | val_ce={val_ce_loss:.4f} | "
            f"train_aux={train_aux_loss:.4f} | val_aux={val_aux_loss:.4f} | "
