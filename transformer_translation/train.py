...
def build_vocabs(
    ...
    src_vocab = Vocab.build(src_tokenized, min_freq=config.min_freq)
    tgt_vocab = Vocab.build(tgt_tokenized, min_freq=config.min_freq)
...
def build_dataloaders(
    ...
    collate_fn = build_collate_fn(pad_id=src_vocab.pad_id)
...
def run_epoch(
    ...
    for batch in data_loader:
        ...
        tgt_self_attn_mask = make_decoder_self_attention_mask(
            tgt_input_ids, pad_id=tgt_pad_id
        )
        cross_attn_mask = make_cross_attention_mask(
            tgt_input_ids, src_ids, pad_id=src_pad_id
        )
...
def train_model(config: Config | None = None) -> dict:
    ...
    train_pairs = read_parallel_corpus(config.train_path)
    if not train_pairs:
        raise RuntimeError("Training corpus is empty. Please check data/raw/train.tsv.")
