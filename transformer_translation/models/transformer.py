...
class TransformerTranslationModel(nn.Module):
    ...
    def encode(
        self, src_ids: torch.Tensor, src_mask: torch.Tensor | None = None
    ) -> torch.Tensor:
    ...
    def forward(
        ...
        """
        memory = self.encode(src_ids, src_mask=src_mask)
    ...
    def greedy_decode(
        ...
        Steps:
            ...
            2. Start target sequence with <bos>.
            3. Predict the next token step by step.
            4. Append argmax token to decoder input.
            5. Stop when <eos> is produced or max_len is reached.
        """
        self.eval()
